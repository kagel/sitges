...
def main():
    ...
    parser = argparse.ArgumentParser(description='Process Sitges Film Festival data for a specific year')
    parser.add_argument('year', type=str, help='Year to process (e.g., 2024, 2025)')
    args = parser.parse_args()
    
    year = args.year
    
    # Read the JSON files from the year-specific directory
    try:
        with open(f'{year}/{year}.json', 'r', encoding='utf-8') as f:
            films_data = json.load(f)['films']  # assuming top-level key is 'films'

        with open(f'{year}/categories.json', 'r', encoding='utf-8') as f:
            categories_data = json.load(f)

        with open(f'{year}/list.json', 'r', encoding='utf-8') as f:
            list_data = json.load(f)

        with open(f'{year}/sessions.json', 'r', encoding='utf-8') as f:
            sessions_data = json.load(f)
    except FileNotFoundError as e:
        print(f"Error: Could not find required files for year {year}")
        print(f"Make sure the directory '{year}' exists and contains the required JSON files.")
        print(f"Missing file: {e}")
        sys.exit(1)
    except json.JSONDecodeError as e:
        print(f"Error: Invalid JSON format in one of the files for year {year}")
    ...
    # Write to CSV file with year-specific filename
    output_filename = f'{year}/composite_sessions_{year}.csv'
    with open(output_filename, 'w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow(header)
        writer.writerows(rows)

    print(f"CSV file '{output_filename}' has been generated successfully.")
